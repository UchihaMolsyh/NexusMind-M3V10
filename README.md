...
<p align="center">
  <img src="https://img.shields.io/badge/NexusMind-v1.0.0-7c5cfc?style=for-the-badge&logo=cpu-intel" alt="NexusMind Badge">
...
```
Upon launching:
1. The model files will be verified.
2. The FastAPI + WebSocket server will start on `http://127.0.0.1:8755`.
3. Your default web browser will open to the Chat UI automatically.

---

## 🛠️ Multi-Model Profiles

You can toggle or switch these profiles dynamically. Configure details in [config.py](file:///c:/Users/uchih/m3v10/config.py):
...
Edit [config.py](file:///c:/Users/uchih/m3v10/config.py) to update system values:
...
*   **Local Execution Sandbox:** The Python executor running in `tools/python_exec.py` is sandboxed using basic restrictions. Avoid executing untrusted user code.
*   **Ethical OSINT:** Integration with tools such as Metasploit, Shodan, and local ports scanners is designed for educational/testing purposes only. Always obtain permission before scanning external targets.
*   **Media APIs:** External requests to HuggingFace for video/image generation use the `HF_API_KEY` defined in `config.py` or configured via system environment variables.
